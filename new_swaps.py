...
def penalty_sum(route, requestnode): #係数ありの評価関数、係数、係数なしの評価関数を返す(全体)
    ...
    penalty = 80* q_s + 50 * d_s + 20 * t_s + h_s
    no_penalty = c_s + 80*q_s + 50*d_s + 20*t_s + h_s
    parameta[0] = q_s
    parameta[1] = d_s
    parameta[2] = t_s
    parameta[3] = h_s
...
def penalty_sum_route_k(route_k, requestnode):  #係数ありの評価関数(1台あたり)
    ...
    penalty = c_s + 80 * q_s + 50 * d_s + 20 * t_s
    return penalty
...
def zentansaku_taboo(initial_Route, kinbo_cost, zentansaku_taboo_list, best_neighbour):
    ...
    for i in riyoukyaku_list:
        ...
        for j in syaryo_loop:
            check = tabu_check(i, j, zentansaku_taboo_list)
            if not check > 0:
    ...
    return NextRoute, kinbo_cost, zentansaku_taboo_list, best_neighbour
...
def main(LOOP):
    ...
    zentansaku_theta = 10
    kinsi = theta
    zentansaku_kinsi = zentansaku_theta
    tabu_list = np.zeros((theta, 3)) - 1
    zentansaku_taboo_list = np.zeros((zentansaku_theta, 3)) - 1
    ...
    while True: #近傍探索　
        ...
        if change_count >= 100:
            zentansaku = copy.deepcopy(zentansaku_taboo(initial_Route, kinbo_cost, zentansaku_taboo_list, best_neighbour))
            kinbo_cost = zentansaku[1]
            NextRoute = copy.deepcopy(zentansaku[0])
            zentansaku_taboo_list = copy.deepcopy(zentansaku[2])
            best_neighbour = copy.deepcopy(zentansaku[3])
            tabu_update_ver2(zentansaku_kinsi, zentansaku_taboo_list, best_neighbour)
        ...
        if loop == 0:
            ...
            saiteki = penalty_sum(saiteki_route, n)
        elif kinbo_cost <= opt:
            opt = kinbo_cost
            saiteki_route = copy.deepcopy(NextRoute)
        ...
        swap_route = copy.deepcopy(swap(saiteki_route, n))
    ...
    print(FILENAME)
    print('探索回数', tansaku)
    np.savetxt('/home/rei/ドキュメント/data_darp01/new_swaps.ods', data, delimiter=",")
